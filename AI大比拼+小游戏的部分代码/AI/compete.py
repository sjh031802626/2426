...
def ai_test(url_get, url_post, teamid, token):
    ...
    swap_scheme = challenge_resp['data']['swap']  # 强制交换最初方案
    ...
    submit_resp = post_submit(url_post, uuid, teamid, token, path, solve.swap_scheme)
    print("submit_resp: \n", submit_resp)
    return submit_resp
...
if __name__ == '__main__':
    ...
    print("question: \n", question)
    # n = int(input())  # 赛题列表的第n题，n从0开始
    # uuid = question[n]['uuid']  # 赛题列表的第一题的uuid
    # record = get_record(R+uuid)  # 返回uuid赛题的记录列表
    # print("record: \n", record)

    # 创建赛题
    # create_resp = post_create(C, teamid, letter, exclude, challenge, step, swap, token)
    # print("create_resp: \n", create_resp)
    ...
    # 查看指定队伍信息
    # team_detail = get_teamdetail(T+str(teamid))
    # print("team_detail: \n", team_detail)
    ...
    '''
    # 挑战赛题,返回uuid题目内容
    challenge_resp = post_challenge(ST+uuid, teamid, token)
    # print("challenge_resp: \n", challenge_resp)
    # print("第几步进行强制转换: ", challenge_resp['data']['step'])
    # print("调换的图片编号: ", challenge_resp['data']['swap'])
    # print("题目标识: ", challenge_resp['uuid'])
    uuid = challenge_resp['uuid']  # 题目标识
    step_num = challenge_resp['data']['step']  # 第几步进行强制交换
    swap_scheme = challenge_resp['data']['swap']  # 强制交换最初方案
    # 题目中图片编号1~9，程序中为0~8，故调整一下调换图片的编号
    ...
    solve.swap_scheme[0] += 1
    solve.swap_scheme[1] += 1
    submit_resp = post_submit(SU, uuid, teamid, token, path, solve.swap_scheme)
    print("submit_resp: \n", submit_resp)
    '''
