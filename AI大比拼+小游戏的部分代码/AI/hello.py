...
class IDAstar:
    ...
    def dfs(self, now, lastd, n):
        ...
        if self.no_swap_exe and n == self.step_num:
            # print("n: ", n, " step_num: ", self.step_num, "no?", self.no_swap_exe)
            # print("before swap: ", Judge_even(now.stat))
            ...
            # print("now.stat_solvable: ", Judge_even(now.stat))
            ...
            # print("init.stat_solvable: ", Judge_even(self.init.stat))
            ...
            for j in range(3):
                for k in range(3):
                    print("\t", now.stat[3 * j + k], end=" ")
                print()
            '''
            return True
            # self.swap_scheme = scheme  # 记录交换方案

        # print(now.stat, "n: ", n, " maxdep: ", self.maxdep, " now.cost: ", now.cost)
        ...
        for i in range(4):
            ...
            if (lastd == -1) or (lastd % 2) != (i % 2) or (lastd == i):
                ...
                if self.dfs(temp, i, n+1):
                    '''
                    print("n: ", n+1, " d: ", temp.prepath)
                    for j in range(3):
                        for k in range(3):
                            print("\t", stat[3 * j + k], end=" ")
                        print()
                    '''
    ...
    def IDA(self):
        ...
        for j in range(3):
            for k in range(3):
                print("\t", self.init.stat[3 * j + k], end=" ")
            print()
        print("self.init.step: ", self.init.step)
        print("self.init.pos: ", self.init.pos)
        print("self.init.cost: ", self.init.cost)
        print("self.init.prepath: ", self.init.prepath)

        print(self.path, self.swap_scheme)
        '''
...
if __name__ == '__main__':
    ...
    # print("swap: ", type(solve.swap_scheme))
